...
init_cond = torch.tensor([0., 0.], dtype = dtype).to(device)

time_interval = torch.linspace(0., 1.5, steps = 500, dtype = dtype).to(device)
time_interval_constrained = torch.linspace(0., 2., steps = 500, dtype = dtype).to(device)

x_1_ref_val = 1.0
x_2_ref_val = 0.0

x_1_ref = torch.tensor([x_1_ref_val], dtype = dtype).to(device)
x_2_ref = torch.tensor([x_2_ref_val], dtype = dtype).to(device)

x_ref = torch.tensor([x_1_ref_val, 0.0], dtype = dtype).to(device)

def objective(x: torch.tensor, u: torch.tensor) -> torch.tensor:
    result = (x[..., 0] - x_1_ref) ** 2 + 1e-6 * (x[..., 1] - x_2_ref) ** 2
    return torch.sum(result, dim = -1).unsqueeze(-1) # stage cost
    return (x[-1, 0] - x_1_ref) ** 2 # terminal cost

def potential_objective(x: torch.tensor, u: torch.tensor) -> torch.tensor:
    return (x[..., 0] - x_1_ref) ** 2 # + 1e-6 * (x[..., 1] - x_2_ref) ** 2
...
double_integrator_lya_obj = Case_Study(
    init_cond,
    time_interval,
    dynamics,
    objective,
...
double_integrator_lya_obj_constrained = Case_Study(
    init_cond,
...
double_integrator_node_obj = Case_Study(
    init_cond,
    time_interval,
    dynamics,
    objective_node,
