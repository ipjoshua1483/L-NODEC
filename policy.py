...
class Policy(nn.Module):
    ...
    def train_policy(
        ...
        for epoch in range(num_epochs):
            ...
            objective = self.objective(trajectory[:-1], case_study)
            loss = None
            if node:
                loss = objective
            else:
                pointwise_loss = self.pointwise_lyapunov_loss(trajectory[:-1], case_study, kappa = 1.)
                pointwise_loss_penalty = self.pointwise_lyapunov_loss_penalty(trajectory[:-1], case_study)
                # print(pointwise_loss_penalty)
            ...
            [states, inputs, objectives] = torch_to_numpy([trajectory, self(trajectory[:-1]), objective])
    ...
    def generate_adversarial_trajectories(
        ...
        case_study: Case_Study, 
        num_ICs: int = 10, 
        perturbation: torch.tensor = torch.tensor([0.1]), 
        seed: int = 42, 
        node: bool = False
    ):
        ...
        for i, init_cond in enumerate(init_conds):
            trajectory = odeint(lambda t, x: case_study.dynamics(x, self(x).squeeze(), t), init_cond, case_study.time_interval, method = 'rk4')
            ...
            if node == False:
                ...
                if len(potential_traj.shape) == 1:
                    ...
                    potential_traj_norm = potential_traj_norm / potential_traj[..., 0]
                [potential_traj_norm] = torch_to_numpy([potential_traj_norm])
                potential_norm_all.append(potential_traj_norm)
        ...
        if node:
            ...
            return time_interval_np, trajectory_all, input_all, terminal_states_mean_np, terminal_states_std_np, constraint_violation_count, potential_norm_all
    
    def generate_vector_fields(self, case_study: Case_Study):
        x1 = np.linspace(-0.5, 1.5, 10)
        x2 = np.linspace(-0.5, 3.5, 20)
        X1, X2 = np.meshgrid(x1, x2)
        X1_tensor, X2_tensor = torch.tensor(X1, dtype = torch.float64), torch.tensor(X2, dtype = torch.float64)
